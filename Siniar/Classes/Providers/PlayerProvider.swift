...
extension NSNotification.Name{
    static let PlayerProviderStateDidChange: NSNotification.Name = NSNotification.Name(rawValue: "kPlaterProviderStateDidChange")
    ...
    func isNextPodcastAvailable() -> Bool{
        ...
        func podcastNext(){
            ...
            guard let playlistSafe = self.playlist,
                  ...
                  newCurrentIndex <= audios.count - 1 else{
                      return
                  }
            self.currentIndex = newCurrentIndex
            self.loadPodcast(audio: audios[newCurrentIndex])
    ...
    @objc private func playerItemDidReadyToPlay(_ sender: Notification){
        if let item = sender.object as? AVPlayerItem{
            if item.status == .readyToPlay {
                ...
                print("something went wrong. player.error shouls contain some information")
            }
        }
    }
